...
pub fn has_invariant_tsc() -> bool {
    ...
    */
    ...
    let CpuidResult { eax: max_basic_leaf, .. } = __cpuid(0_u32);
    ...
    let CpuidResult { edx, .. } = __cpuid(0x8000_0007_u32);
...
impl Frequency {
    ...
    #[cfg(feature = "std")]
    pub fn estimate_over(over: std::time::Duration) -> Self {
        let wall_start = std::time::Instant::now();
        let start = Start::now();
        std::thread::sleep(over);
        let stop = Stop::now();
        let wall_elapsed = wall_start.elapsed();
        let cycles = (stop - start).cycles();
...
impl Start {
    ...
    /// Convenience method to get cycles since a `Start`.
